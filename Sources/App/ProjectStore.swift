...
enum ProjectStore {
    ...
    static var currentProjectIDKey: String { "tool.project.id" }

    /// 项目列表文件:`~/.open-pet-agent/projects.json`(顶层 index)。
    static var projectListURL: URL {
    ...
    static func current(defaults: UserDefaults = .standard) -> AgentProject {
    ...
    /// 设置当前选中项目。
    static func setCurrent(_ id: String, defaults: UserDefaults = .standard) {
    ...
    /// 返回新项目(已 ensure)。
    @discardableResult
    static func create(name: String) throws -> AgentProject {
    ...
    static func ensureDefaultProjectRegistered(defaults: UserDefaults = .standard) {
