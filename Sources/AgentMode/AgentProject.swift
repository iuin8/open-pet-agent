...
/// PetAgent agent 工作项目(P1a 多项目数据地基,在 AgentMode target 供 App/ACPSmoke 共用)。
///
...
/// - 外部项目(`isExternal=true`,P1c):用户外部目录(如 `~/work/my-app/`),跟项目走(VSCode 模式)。
///
/// 详见 `docs/project-config-architecture-design.md`。
public struct AgentProject: Identifiable, Codable, Sendable, Equatable {
    ...
    public init(id: String, name: String, rootURL: URL, isExternal: Bool, createdAt: Date) {
        self.id = id
        self.name = name
