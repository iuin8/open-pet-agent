...
public struct ProjectCapabilityAuditStore: Sendable {
    ...
    public static func isGeneratedTarget(_ url: URL) -> Bool {
        ...
        if url.lastPathComponent == "opencode.json" {
            let marker = url.deletingLastPathComponent().appendingPathComponent(".open-pet-agent-generated.opencode")
            return FileManager.default.fileExists(atPath: marker.path)
        }
        return false
    }
    ...
    private func validateRestore(_ backup: CapabilityAuditState.Backup, project: AgentProject) throws {
        ...
        guard !FileManager.default.fileExists(atPath: target.path) || Self.isGeneratedTarget(target) else {
            throw ProjectCapabilityAuditError.unownedRestoreTarget(target)
        }
    }
    ...
    private func backup(
        ...
        guard Self.isGeneratedTarget(target) else { throw ProjectCapabilityAuditError.unownedRestoreTarget(target) }
    ...
    public func diagnostics(project: AgentProject, plans: [ProjectionPlan]) throws -> [ProjectConfigDiagnostic] {
        ...
        let diagnostics = plans.flatMap { plan in
            plan.operations.compactMap { operation -> ProjectConfigDiagnostic? in
                ...
                guard !ProjectCapabilityAuditStore.isGeneratedTarget(target) else { return nil }
