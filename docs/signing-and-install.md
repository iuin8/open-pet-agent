...
1. `./build.sh` — `swift build -c release` → 打 `OpenPetAgent.app` bundle →
   `xattr -cr` 清扩展属性 → `codesign` 用 Apple Development 证书签名
...
`build.sh` 检测不到证书时 fallback ad-hoc 签名 + 警告:

```
🔐 未找到 Apple Development 证书,退回 ad-hoc 签名
   ⚠️  每次重 build 后 CDHash 变,可能需要重新授权 Accessibility/截屏
   💡 申请免费 Apple ID 开发者账号可解决: https://developer.apple.com
```
